...
system.time({
    ...
    # 如果user.id重复，那么取last.status.id大的，即更新的
...
system.time({
    ...
    r.user.info.weibo.1810[, uniqueN(user.id)] == nrow(r.user.info.weibo) # 验证每个user.id只占一行
...
}) # 6min@1e6 (batch.size = 1e6 比 1e7要快，因为1e7会撑爆内存)

# 添加 lastcrawl，本次为 1810
...
# 这步非常吃内存，因此必须清空内存，重启mongodb service！
library(mongolite)
